...
def prediction():
    ...
    (
        ...
        test_sample_time,
        test_label_time,
    ) = utility.test_split_sequence(test_sequence=test_df, w=w, p_w=p_w)

    test_sample = test_sample.reshape(
        (test_sample.shape[0], test_sample.shape[1], n_features)
    )
    ...
    tmp_result_df["Pred Error"] = abs(
        tmp_result_df["timestamp_label"] - tmp_result_df["timestamp_prediction"]
    ).to_numpy()
    ...
    deepant_error_threshold = (
        deepant_result_df["Pred Error"].mean()
        + 3 * deepant_result_df["Pred Error"].std()
    )
    ...
    deepant_result_df["Window Mean"] = (
        deepant_result_df["Pred Error"]
        .rolling(window=deepant_window, min_periods=1)
        .mean()
    )
    deepant_result_df["Window Std"] = (
        deepant_result_df["Pred Error"]
        .rolling(window=deepant_window, min_periods=1)
        .std()
    )
    deepant_result_df["Upper Bound"] = (
        deepant_result_df["Window Mean"]
        + deepant_std_coef * deepant_result_df["Window Std"]
    )
    deepant_result_df.drop(["Window Mean", "Window Std"], axis=1, inplace=True)

    deepant_result_df["Anomaly_Predict"] = (
        deepant_result_df["Pred Error"] >= deepant_result_df["Upper Bound"]
    )
    deepant_result_df["Static Predict"] = (
        deepant_result_df["Pred Error"] > deepant_error_threshold
    )

    deepant_result_df.set_index(["datetime"], inplace=True)

    # get range_proba function
    modified_static = get_range_proba(
        deepant_result_df["Static Predict"].astype("int64"), deepant_result_df["label"]
    )
    deepant_result_df["Modified Static"] = modified_static

    modified_dynamic = get_range_proba(
        deepant_result_df["Anomaly_Predict"].astype("int64"), deepant_result_df["label"]
    )
